...
<%
    ...
    String pwd = "123456";
    String jdbcUrl = "jdbc:mysql://localhost:3306/CHK?serverTimezone=Asia/Shanghai&useSSL=true";
    ...
    ResultSet rs = null;//ResultSet是一个指向数据库的变量，本质上是不保存任何数据的，执行查询
%>
<div class="head">
...
<div class="excel" style="height: 95%; overflow: scroll;">
    <table style="table-layout: fixed;">
        ...
        <%
            sql = "select * from goods";//定义sql语句
            assert con != null;
            try {
                ps = con.prepareStatement(sql);//执行sql语句
                rs = ps.executeQuery();//执行后的结果集
                assert rs != null;
                while (rs.next()) {
        %>
        <tr>
            ...
            <td style="white-space: nowrap;text-overflow: ellipsis;overflow: hidden;"><%=rs.getString("image")%>
            </td>
            <td style="color: red;"><%=rs.getInt("price")%>¥</td>
            <td><%=rs.getString("sort")%>
            </td>
            ...
            <td>
                ...
                <button style="background-color: red;"
                        onclick="window.location.href='goods_mysql/delete.jsp?id=<%=rs.getString("id")%>'">删除
                </button>
            </td>
        </tr>
        <%
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            <form action="goods_mysql/upData.jsp" enctype="multipart/form-data">
                <%
                    int goodId = Integer.parseInt(request.getParameter("param"));
                    sql = "select * from goods where id = ?";//定义sql语句
                    assert con != null;
                    try {
                        ps = con.prepareStatement(sql);//执行sql语句
                        ps.setInt(1, goodId);
